...
export async function POST(request: Request) {
  ...
  const t1 = Date.now();
  console.log("[correct-capability][timing] after request.json() t1=" + t1 + " delta=" + (t1 - t0) + "ms bodyKeys=" + (body ? Object.keys(body).join(",") : "null") + " bodySize=" + (body ? JSON.stringify(body).length : 0));
  ...
  const t2 = Date.now();
  console.log("[correct-capability][timing] after getUser() t2=" + t2 + " delta=" + (t2 - t1) + "ms");
  if (userError || !user) {
  ...
  const t3 = Date.now();
  console.log("[correct-capability][timing] after profile query t3=" + t3 + " delta=" + (t3 - t2) + "ms docCount=" + (profile.document_metadata?.length ?? "null"));
...
Cross-reference the self-reported capability tags provided in this profile against the evidence above. For each tag that describes a capability not already captured by an existing entry, add it as its own entry, applying the same Tagging rules used above: if an official uploaded document, diploma, professional certification, DD214, military service record, or professional license, directly supports that specific capability, tag it [VERIFIED]; otherwise tag it [USER_PROVIDED]. Do not default a cross-referenced tag to [USER_PROVIDED] without first checking whether supporting documentary evidence exists for it. If a tag is already covered by an existing VERIFIED or USER_PROVIDED entry, do not duplicate it.

Every capability name must be written in plain business language any reader outside that person's specific field, branch, or trade would immediately understand, describing what they can do or deliver, never a duty title, role name, school name, qualification name, or internal jargon specific to their branch, trade, or employer (for example, "Master Breacher," an MOS code, or an internal job title). If a reader with zero background in that field would not understand the capability name on its own, with no other context, rewrite it. The specific role, school, qualification, or internal title belongs in the description as supporting evidence, not in the name itself.
...
Respond with only the five sections above. No preamble, no closing remarks.`;
  ...
  const t3b = Date.now();
  console.log("[correct-capability][timing] before doc loop t3b=" + t3b + " delta=" + (t3b - t3) + "ms storedDocCount=" + storedDocs.length);
  ...
  const t4 = Date.now();
  console.log("[correct-capability][timing] after doc loop t4=" + t4 + " delta=" + (t4 - t3b) + "ms extractedCount=" + extractedTexts.length + " rawBlocks=" + rawDocBlocks.length + " unreadable=" + unreadableDocLabels.length);
  ...
  const t6 = Date.now();
  console.log("[correct-capability][timing] before Sonnet synthesis t6=" + t6 + " delta=" + (t6 - t5b) + "ms hasPdfs=" + hasPdfs + " docBlockCount=" + docBlocks.length + " promptLen=" + fullPrompt.length);
  ...
  try {
    ...
    const t6b = Date.now();
    console.log("[correct-capability][timing] Sonnet call FAILED t6b=" + t6b + " delta=" + (t6b - t6) + "ms");
  ...
  const t7 = Date.now();
  console.log("[correct-capability][timing] Sonnet complete t7=" + t7 + " delta=" + (t7 - t6) + "ms responseLen=" + text.length);
